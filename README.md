...
You can mount an `.sql` file into the /docker-entrypoint-initdb.d/ directory to add additional data.  Some [files](https://github.com/JeffersonLab/mycontainer/tree/main/docker-entrypoint-initdb.d) already exist, so be careful not to overwrite or name your file starting with a number that forces it to be processed before the DDL is run.   Use a file mount and avoid an entire directory mount else you'll erase the `00_base.sql` file needed to setup the base DML. 
